  ...
  return (
    ...
    <div className="flex-grow px-3">
        {cartItems.length === 0 ? (
            ...
            {cartItems.map(item => (
                <div className="" key={item.id}>
                    <div className="flex rounded-md overflow-hidden items-center w-full bg-white mb-3 h-24 shadow-md">
                        <img src={item.image} alt={item.name} className='w-24 h-24 p-3 bg-gray-400' />
            ...
            <button onClick={()=> navigate("/checkout")} className=" py-3 mt-2 font-semibold rounded-md  border-2 border-gray-600 bg-white w-full text-center">
               PROCEED TO CHECKOUT
            </button>
        </div>
        )}
